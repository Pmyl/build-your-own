...
fn huffman_cli_impl(args: &[&str], input: impl Read, output: impl Write) {
    ...
    let mut targets = HuffmanTargets::new(options.input_file, input);

    let mut output: Box<dyn Write> = if let Some(file) = options.output_file {
        Box::new(std::fs::File::open(file).expect("file not found"))
    } else {
        Box::new(output)
    };
...
impl Display for Bits {
    fn fmt(&self, fmt: &mut Formatter<'_>) -> Result<(), std::fmt::Error> {
        ...
        for i in 0..self.amount_of_bits {
...
impl Bits {
    ...
    fn byte(byte: u8) -> Self {
        Self { data: byte, amount_of_bits: 8 }
    }
...
impl<T: Write> BitsWriter<T> {
    ...
    fn write(&mut self, bits: &Bits) {
        for i in 0..bits.amount_of_bits {
            ...
            self.current_byte = (((bits.data << i) >> self.shift) & self.mask) | self.current_byte;
...
impl<T: Read> BitsReader<T> {
    ...
    fn read(&mut self) -> bool {
        if self.mask == 0b00000000 {
            let mut buf = [0u8; 1];
            self.reader.read(&mut buf).expect("to read");
...
fn write_huffman_file(input: &mut (impl Read + ?Sized), output: &mut (impl Write + ?Sized), table: HuffmanPrefixCodeTable, root: HuffmanNode) {
    ...
    // TODO: writing only bytes creates a padding of zeroes at the end every time we end up with
    // bits to write not as a multiple of 8. The only way to solve it is to
    // find a way to actually write bits into the file and not bytes

    // IDEA: the BitsWriter puts one last byte at the end of the output to indicate
    // how many bits of padding there are, BitsReader takes in account that and doesn't read
    // the padding bits. This means that BitsWriter/BitsReader are not generic writer/reader, they only
...
mod tests {
    ...
    fn bit_write_and_read() {
        ...
        bits = bits.add(false);
        bits = bits.add(true);
        // TODO: Bits::add should fail if add more than 8 bits
    ...
    fn encode_decode_should_return_original_input() {
        ...
        assert_eq!(String::from_utf8(output.clone()).expect("to do it"), "tests".to_string());
    }
}
